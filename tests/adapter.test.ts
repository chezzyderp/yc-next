...
async function createFakeNextBuild(root: string) {
  ...
  await writeFile(path.join(serverDir, "app/users/route.js"), "module.exports = {};");
  await writeFile(path.join(standalone, ".next/server/app/users/route.js"), "module.exports = {};");
  await writeFile(path.join(standalone, "package.json"), '{"type":"commonjs"}');
  await writeFile(path.join(standalone, "server.js"), "// fake standalone entry");
}

describe("yandexCloudAdapter", () => {
  it("modifies config to standalone", () => {
    const adapter = yandexCloudAdapter();
    const config = adapter.modifyConfig({ experimental: { existingFlag: true } });

    expect(config.output).toBe("standalone");
    expect(config.skipMiddlewareUrlNormalize).toBe(true);
    expect(config.experimental).toEqual({ existingFlag: true });
  });
  ...
  it("builds route bundles", async () => {
    ...
    expect(zip.getEntry("runtime/next-server.js")).toBeDefined();
  ...
  it("builds single-function router when configured", async () => {
    ...
    const manifest = JSON.parse(
      await fs.readFile(path.join(projectDir, ".next/yc/manifest.json"), "utf8"),
    );
